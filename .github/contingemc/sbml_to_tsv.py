...
def find_sbml_file(directory):
    ...
    if len(SBML_file) > 1:
        ...
        sys.exit(
            "ERROR: No XML files found. Is this being run from the correct directory?"
        )
...
def generate_headers(section):
    ...
    for entity in children:
        entity_annotations = entity.xpath(
            ".//sbml:annotation//dc:identifier", namespaces=nsmap
        )
        for resource in entity_annotations:
            if (
                resource := resource.attrib["{%s}title" % nsmap["dc"]]
            ) not in annotations:
                annotations.append(resource)
        entity_notes = entity.xpath(".//sbml:notes//xhtml:p", namespaces=nsmap)
        for note in entity_notes:
            if (note := note.text.split(":")[0]) not in notes and note not in [
                "Comment",
                "Curator",
            ]:
                notes.append(note)
...
def pull_db_refs_from_sbml(dbs, entity):
    ...
    for i in dbs:
        ...
        if "!Identifier" in i:
            ref = entity.xpath(
                ".//dc:identifier[@dc:title='%s']" % search_term, namespaces=nsmap
            )
            if len(ref):
...
SBtabHeaderTemplate = Template(
    "!!SBtab TableID='$TableID' Document='%s' TableType='$TableType' TableName='$TableName' %s Date='%s'"
    % (NAME, SBtabVersionString, DATE)
)
...
try:
    curators = model.xpath(".//vCard:vcards/vCard:vcard", namespaces=nsmap)

    # if curators were found
    if len(curators):
        # check what information is included in the SBML

        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="curator", TableType="Curator", TableName="Curators"
        )
...
try:
    ...
    if len(genes):
        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="compound", TableType="Gene", TableName="Genes"
        )
        dbs = generate_headers(genes)
        headers = (
            ["!ID", "!Symbol", "!LocusName", "!Name"] + dbs + ["!Curator", "!Comments"]
        )
...
try:
    ...
    if len(pathways):
        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="pathway", TableType="Pathway", TableName="Pathways"
        )
...
try:
    ...
    if len(compartments):
        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="compartment", TableType="Compartment", TableName="Compartments"
        )
        dbs = generate_headers(compartments)
        headers = (
            ["ID", "!Name", "!Size", "!spatialDimensions"]
            + dbs
            + ["!Curator", "!Comments"]
        )
        with open(OUTPUT_LOCATION / "Compartment-SBtab.tsv", "w+", newline="") as f:
            ...
            children = compartments.xpath(
                "./*[not(self::sbml:notes)]", namespaces=nsmap
            )
...
try:
    ...
    if len(compounds):
        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="compound", TableType="Compound", TableName="Compounds"
        )
...
try:
    ...
    if len(parameters):
        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="parameter", TableType="Parameter", TableName="Parameters"
        )
        dbs = generate_headers(parameters)
        headers = (
            ["!ID", "!Parameter", "!Value", "!Unit", "!Type", "!SBOTerm"]
            + dbs
            + ["!Curator", "!Comments"]
        )
...
def extract_genes(fragment):
    if len(fragment.getchildren()):
        ...
        if (
            clean_operator(fragment.getparent().tag)
            not in [operator, "geneProductAssociation"]
            and clean_operator(fragment.tag) != "geneProductAssociation"
        ):
            inner = "(" + inner + ")"
        return inner
    else:
...
def extract_formula(fragment):
    ...
    if reactants is not None:
        ...
        reactants = [
            react.attrib["species"]
            if int(float(react.attrib["stoichiometry"])) == 1
            else react.attrib["stoichiometry"] + " " + react.attrib["species"]
    ...
    if products is not None:
        ...
        products = [
            prod.attrib["species"]
            if int(float(prod.attrib["stoichiometry"])) == 1
            else prod.attrib["stoichiometry"] + " " + prod.attrib["species"]
...
try:
    ...
    if len(reactions):
        SBtab_header = SBtabHeaderTemplate.substitute(
            TableID="reaction", TableType="Reaction", TableName="Reactions"
        )
        ...
        with open(OUTPUT_LOCATION / "Reaction-SBtab.tsv", "w+", newline="") as f:
            ...
            for r in children:
                ...
                pathways = r.xpath(
                    ".//xhtml:p[contains(text(),'Pathway:')]", namespaces=nsmap
                )
                ...
                data.extend(
                    [i for i in pull_db_refs_from_sbml(dbs, r) if "Pathway:" not in i]
                )  # extra filter to prevent pathway double annotation
